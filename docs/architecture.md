...
React 웹뷰는 FastAPI가 제공한다. 큐와 감사 기록은 MySQL에 저장하여 Redis·별도 메시지 브로커 없이 명령을 복구한다. Freqtrade의 Telegram·모의매매·백테스트 사용 흐름을 참고하되 주문 규칙은 토스의 실제 제약에 맞췄다. 업비트 원화 마켓은 별도 어댑터로 연결한다. Binance 주문 코드는 포함하지 않는다.
...
- 업비트는 `identifier`로 주문을 조회하고 모호한 POST를 재전송하지 않는다.
- 결과가 불명확한 주문이 있으면 같은 시장의 다른 종목 새 주문도 막는다. 전략이 중단되었거나 유효시간을 넘긴 경우 재전송하지 않는다. 증권사 주문 번호를 사용한 수동 대조가 필요하다.
...
- 기존 보유분이나 수동 주문과 봇 수량이 맞지 않으면 해당 전략을 중단한다. 외부 주문을 자동 취소하거나 사용자 주식을 자동 편입하지 않는다. 업비트는 명시적으로 준비·확인한 보유 전량 그리드 초안을 시작할 때만 초기 코인을 편입한다. 각 단계의 수량과 순매도대금은 같은 전략 JSON 장부에서 함께 갱신한다.
- 웹소켓은 시세 전달에 사용한다. 원장 상태는 REST로 대조한다. 재연결 시 구독과 시세를 복구하며 과거 이벤트가 재생된다고 가정하지 않는다.

## 승인과 보안

로컬 UI는 loopback만 허용한다. 운영 UI는 GitHub OAuth의 허용 사용자 ID를 확인한다. Telegram 인증을 선택하면 `initData` 서명·5분 유효시간·허용 사용자 ID를 확인한다. 세션은 Secure·HttpOnly 쿠키를 사용한다. 변경 요청은 Origin을 확인한다. Telegram은 사용자 ID·개인 대화 ID를 모두 검사한다.
...
평가금액은 **봇에 배정한 자산**의 현금과 보유 주식 평가 합계다. 증권사 계좌 전체 자산·입출금·배당 자동 회계가 아니다. 시세가 없는 보유분은 평가 불완전으로 표시하고 신규 주문을 보류한다. 토스 USD와 업비트 KRW 예산·손익은 별도로 계산한다.
